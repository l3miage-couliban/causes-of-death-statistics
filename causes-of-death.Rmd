...
## Contexte du problème
  ...
  1) representation sur carte graphique des décès en fonction par cause et par année
  2) Les 5 principales causes sur des regions selectionnées
  3) L'évolution des décès et de la démographie sur des regions de 1990 à 2019 
...
# Représentation surla carte géographique des décès en fonction par cause et par année
```{r}
...
ui <- page_sidebar(
  title = "Représentation sur carte graphique des nombres de décès dans le monde par causes et par années",
  sidebar = sidebar(
    helpText("Variables"),
    selectInput(
      "cause",
      label="Causes de décès",
      choices = causesName,
...
## Ces sous-régions de chaque continent avec 5 causes de décès populaires
...
ggplot(RegionNorthernAmerica , aes(x=`Causes name`, y=Death_numbers, fill=`Causes name`)) +
  ...
  guides(fill=FALSE)
```
# Courbes d'évolution du nombre de décès au cours des années de certaines régions
...
plotSubSaharanAfrica <- africaData %>%
  ...
  ggplot(aes(x=Year)) + 
  geom_line(aes(y = death_numbers, color="Décès")) +
  geom_line(aes(y = population, color="Population"))+
  scale_color_manual(values = c("Décès" = "red", "Population" = "blue")) +
...
plotNorthernAmerica <- americaData %>%
  ...
  ggplot(aes(x = Year)) + 
  geom_line(aes(y = death_numbers, color = "Décès"), size = 0.5) +
