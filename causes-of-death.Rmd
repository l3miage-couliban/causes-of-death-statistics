...
```{r}
deathNumberInOceania <- data %>%
  filter(region=="Oceania") %>%
  group_by(Year) %>%
  summarise(death_numbers=sum(`Death Numbers`, na.rm = TRUE)) %>%
  mutate(divide_per_one_million=death_numbers / 1000);

deathNumberInOceania %>%
  ggplot() + 
  aes(x = Year, y = divide_per_one_million) +
  geom_line(stat = "identity", width = 0.5) +
  labs(
    title = "Evolution du nombre de décès en Océanie",
    x = "Années",
    y = "Nombre de décès (en milliers)"
  );
```
...
# SUMMURIZE ALL
```{r}
sumEuropePopulation <- data %>% filter(region=="Europe") %>%
...
europeData %>%
  ggplot(aes(x=Year)) + 
  geom_line(aes(y = death_numbers), stat = "identity", width = 0.5) +
  geom_line(aes(y = population), stat = "identity", width = 0.5)+
  scale_y_continuous(
